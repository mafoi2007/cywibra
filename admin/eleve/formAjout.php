<h1>Ajout d'élèves à la BD</h1>
	
	
	<form method ="POST" action="../traitement.php">
		<?php 
			$nbr = $config->getCode();
			if(strlen($nbr)==1){$lastNbr = '000'.$nbr;}
			elseif(strlen($nbr)==2){$lastNbr = '00'.$nbr;}
			elseif(strlen($nbr)==3){$lastNbr = '0'.$nbr;}
			elseif(strlen($nbr)>=4){$lastNbr = $nbr;}
			// echo $lastNbr;
			$var = "D";
			$var .= DATE('Y');
			$var .= "-";
			$var .= $lastNbr+1;
			$lastNbr+=1;
			echo "<input type='hidden' name='numero' value='".$lastNbr."' />";
			?>
